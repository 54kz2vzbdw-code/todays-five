...
| Real Supabase `tools/realsync4.js` | Deferred to after the deploy: the server allows 12 new lists an hour per address and the afternoon spent them (the first run, the staged 1.2 device, the probes), so the suite runs with the live checks once the window reopens; the result is in the live-check section below. The first run on 1.3 passed its first three checks (envelopes on the wire, a view link's put refused with 403, the unchanged poll) before failing on the old five-line expectation, since retargeted to three |
