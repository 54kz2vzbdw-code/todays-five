...
| Paired simulators | `watchsim.mjs all` — iPhone 17 + Apple Watch Series 11 (46mm) on iOS 26.5 / watchOS 26.5, 9 steps ok. `-TFFontSelfTest`: **kits=16 pairs=15 files=35 bundled=35/35, pass=102/102** (was 96/96 over 33 faces at b216), Caveat 500 and 700 among them. `-TFWatchDemo -TFWatchSelfTest`: 14 checks green (finale fired after 0.342 s, shuffle ×10 distinct=5, haptics check=10 uncheck=2 finale=1 shuffle=10, appGroup=true, confetti 147 frames over 2.47 s = 59.0/s, theme change harbor→terminal). `-TFAddSelfTest` pass=13/13. `-TFConfettiSelfTest` pass=96/96. `-TFFaceProbe` facesOnDisk=35 registered=35, the two renderings identical as before. The unlock-on-the-phone check (the pair appearing in the Watch picker, a complication redrawn) can only run against the deployed site, because the phone app is a WKWebView on the live page — see the row below |
| Paired simulators, after the deploy | The phone app on the live page (build 266): the word typed into Import a code, the Extra group appeared, Whiteboard chosen for Day; a relaunch with `-TFDumpWatchSend` printed `secret: unlocked (secret=false, extra pairs=1), read 2 kit(s) from the page` and `sent … kits=2 keys=at,kits,links,v` (kits=0 before). The Watch relaunched: `heard v=1 … kits=2`, `unlocked=2 offered=18`. `-TFThemeSet night:chalkboard` put Chalkboard on and printed `face: kit=chalkboard pair=chalk … snapshot written, reloadAllTimelines requested`; `-TFKit whiteboard` the same for Whiteboard. Screenshots `apple/shots/watch/today-chalkboard.png`, `today-whiteboard.png`, `theme-picker-extra.png` (the picker's top; its last rows, where the pair sits, need a scroll the harness cannot make). Every tally read over `simctl launch --console-pty` |
