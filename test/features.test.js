...
test("what's new fires on a changed version string, never on its order: a 1.3 device, a 1.2 device, a 1.1 device and a 1.0 device (4.0.0) each see the newest entry once", () => {
  ...
  assert.match(toast, /sharper/i, "the headline says the app got sharper (1.7), not what was found");
  assert.doesNotMatch(toast, /bug|fix|audit|found/i, "and nothing about what was found");
...
test("the changelog (1.2): 1.0 and later only, a one-sentence headline of 12 words or fewer, up to three tagged items of 14 words or fewer, nothing about the plumbing", () => {
  ...
  assert.deepEqual(wn.versions.map(v => v.version), ["1.7", "1.5", "1.4", "1.3", "1.2", "1.1", "1.0"], "the 0.x entries are in CHANGELOG.md, never rendered");
  ...
  for (const v of ["1.7", "1.5", "1.4", "1.3", "1.2", "1.1", "1.0", "0.3", "0.2", "0.1"]) assert.ok(new RegExp("^## " + v.replace(".", "\\."), "m").test(md), "CHANGELOG.md holds " + v);
...
test("the version is one number in three places, the build in four, and there are no dates anywhere", () => {
  ...
  assert.deepEqual(wn.versions.map(v => v.version), ["1.7", "1.5", "1.4", "1.3", "1.2", "1.1", "1.0"], "the public history: 1.0 and later (4.0.0 became 1.0; the pre-releases live in CHANGELOG.md)");
