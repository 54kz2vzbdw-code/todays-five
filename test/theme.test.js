...
test("1.11: light and pink keep v1's primary tokens exactly; Dark is recoloured in place", () => {
  ...
  // 1.12 b207: light's danger moved, and it is the one v1 token that has. #B8402A was 4.12:1 on this
...
test("1.12 b207: the brand constants are still the kits', the mark still carries #A86014, and Paper and Terminal carry their own accents again", () => {
  const paper = CURATED.find(t => t.id === "paper").colors, term = CURATED.find(t => t.id === "terminal").colors, dark = curated("dark").colors;
  // unchanged by 1.12 b207: the brand cannot drift from the kit it grew out of
  ...
  // the mark does not move. 1.12 b207 unpins the *UI* accent; BRAND_ACCENT, the colourways and the
  ...
  // 1.12 b207: the pin is gone. Paper and Terminal carry the accent families they had before 1.11, byte
...
test("1.12 b207: every kit measured against its OWN grounds — accent 3:1 and accent text 4.5:1 on --ink and on --ink-3, printed", () => {
  ...
  // 1.12 b207: danger on --ink-3 too — the re-pointing is what found light's, at 4.12 with #B8402A
...
test("1.7: every curated kit's accent text and danger clear 4.5:1 and its accent 3:1 on the elevated surface too (1.12 b207: no kit is skipped any more)", () => {
  for (const t of CURATED) {
    const c = t.colors;
...
test("1.12 b207: the kit fixture the Swift core and the Watch read still is theme.js", () => {
