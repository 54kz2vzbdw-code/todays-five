...
for (const [label, opts, touch] of VIEWPORTS) {
  ...
  await test(label + ": a 1.0 device (it called itself 4.0.0) opens 1.7 — the toast once, nothing else, nothing about version numbers, list intact, no hints later", async () => {
    const t = await fresh(opts);
    const { listId } = await t.s();
    ...
    assert.ok(new RegExp("New in " + VERSION.replace(".", "\\.")).test(msg), msg); assert.ok(!/4\.0\.0|renumber|1\.1\b|1\.2\b|1\.3\b/.test(msg), "nothing about version numbers: " + msg); assert.ok(/sharper/i.test(msg), "the headline says the app got sharper (1.7)"); assert.equal((await t.page.textContent("#wn-more")).trim(), "What's new");
  ...
  await test(label + ": About shows the version as 1.7 (build N) and the changelog in its shape, no dates", async () => {
    ...
    assert.equal(log.join(","), "1.7,1.5,1.4,1.3,1.2,1.1,1.0", "1.0 and later; the pre-releases never render");
    assert.ok(/Sharper all over\./.test(await t.page.textContent("#log > li:first-child div")), "a headline per version"); assert.ok(/Six more sounds\./.test(await t.page.textContent("#log > li:nth-child(2) div")), "and the one before it");
  ...
  await test(label + ": a 1.1 device opens 1.7 — Follow system and the schedule migrate into the switch, the theme on screen does not change, and the toast is the only new thing", async () => {
    ...
    assert.ok(await t.page.locator("#whatsnew").isVisible(), "the toast"); assert.ok(/New in 1\.7: Sharper all over\./.test(await t.page.textContent("#wn-msg")), "the headline only: " + await t.page.textContent("#wn-msg"));
  ...
  await test(label + ": a 1.3 device opens 1.7 — every list it holds is mine with no question and no groups, and the toast is the only new thing", async () => {
    const t = await fresh(opts);
    ...
    assert.ok(await t.page.locator("#whatsnew").isVisible(), "the toast"); assert.ok(/New in 1\.7: Sharper all over\./.test(await t.page.textContent("#wn-msg")), await t.page.textContent("#wn-msg"));
  ...
  await test(label + ": a page open across a deploy — its later modules are asked for by build and the service worker answers from that build's cache, this build's come fresh, an unknown build falls back to the network; a page whose build cannot be served reloads once after flushing and comes back to its view with the panel it asked for", async () => {
    ...
    const keys = await t.page.evaluate(() => caches.keys()); assert.ok(keys.includes("tf-v1.7-b" + build), "this build's cache: " + keys.join(","));
