...
// is the second monitor's whole day.
//
...
/* ---------------- the conditions ----------------
   ...
   hold it is thrown and marked, never quietly averaged in. */
...
async function trial(browser, cond, phaseMs) {
  ...
  try {
    ...
    let r = await seen();
    if (r.hit) return { ms: 0, live: liveAfterPreroll, errors };
    ...
    while (t < cond.horizon) {
      ...
      if (cond.clickAfterMs && !clicked && t >= cond.clickAfterMs) {
        ...
        if (r.hit) return { ms: t, live: liveAfterPreroll, errors };
      }
    ...
    return { ms: null, live: liveAfterPreroll, errors };      // never, inside the horizon
  } finally {
    await ctx.close();
  }
}
...
for (const c of report.conditions) {
  ...
  console.log(`| ${c.label} | ${trials}${c.broke ? ` (${c.broke} trial(s) could not run)` : ""} | ${c.seen.length ? `${c.underASecond}/${c.seen.length}` : "—"} | ${bound} |`);
}
const notLive = report.conditions.reduce((n, c) => n + c.notLive, 0);
console.log(`\npage errors: ${pageErrors}`);
if (notLive) console.log(`${notLive} trial(s) had stopped believing they were live before the write — worth reading, not an error`);
