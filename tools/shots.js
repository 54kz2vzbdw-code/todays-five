...
for (const [label, opts, touch] of VIEWPORTS) {
  ...
  await step("save-link", async () => { if (await page.$("#w-keep")) { await page.evaluate(() => document.getElementById("w-keep").click()); } else await page.click("#w-new"); /* 1.9: Skip starts an empty list; the shots keep the seed lines the way Keep does */ await page.waitForSelector("#p-save[open]"); await wait(400); await shot("save-link"); if (await page.$("#save-phone:not([hidden])")) { await page.click("#save-phone summary"); await wait(400); await shot("save-link-phone"); } await page.click("#save-done"); await wait(700); });
  ...
  await step("whose", async () => { // 1.4: a second list this device then forgets, opened from a bare link — the one question
    ...
    await openMore("lists"); await page.waitForSelector("#p-lists[open]"); await page.click("#lists-menu .row:has(.cur) .more"); await page.waitForSelector("#p-list[open]"); await page.click('#list-detail-menu [data-lact="rename"]'); await page.waitForSelector("#ask[open]"); await page.fill("#ask-input", "Sarah's groceries"); await page.click("#ask-ok"); await wait(500); // 1.9: the nickname lives in the list's detail
